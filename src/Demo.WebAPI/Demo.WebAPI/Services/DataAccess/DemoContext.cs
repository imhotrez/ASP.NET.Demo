...
namespace Demo.WebAPI.Services.DataAccess {
    public sealed class DemoContext : IdentityDbContext<AppUser, AppRole, long, AppUserClaim, AppUserRole, AppUserLogin,
        ...
        public DbSet<Metadata> FilesMetadata { get; set; }

        protected override void OnModelCreating(ModelBuilder builder) {
            base.OnModelCreating(builder);

            builder.Entity<Metadata>(m =>
            {
                m.HasOne(u => u.User)
                    .WithMany(md => md.ImageMetadatas)
                    .HasForeignKey(x => x.UserId);

                m.ToTable(nameof(ModelBuilder) + 's', "image");
            });
            ...
            builder.Entity<AppRoleClaim>(b =>
                b.ToTable(nameof(AppRoleClaim) + 's', "auth"));
            builder.Entity<AppUserClaim>(b =>
                b.ToTable(nameof(AppUserClaim) + 's', "auth"));
            builder.Entity<AppUserLogin>(b =>
                b.ToTable(nameof(AppUserLogin) + 's', "auth"));
            builder.Entity<AppUserRole>(b =>
                b.ToTable(nameof(AppUserRole) + 's', "auth"));
            builder.Entity<AppUserToken>(b =>
                b.ToTable(nameof(AppUserToken) + 's', "auth"));
            builder.Entity<RefreshSession>(b =>
                b.ToTable(nameof(RefreshSession) + 's', "auth"));
        }

        public override Task<int> SaveChangesAsync(CancellationToken cancellationToken = default) {
            var entities = ChangeTracker
                .Entries()
                .Where(p => p.Entity.GetType()
                                             .GetTypeInfo()
                                             .GetInterfaces()
                                             .Contains(typeof(IDatedEntity)))
                .ToArray();
